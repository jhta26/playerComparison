...
const div1 = div({ class: 'col m6', id: 'div1' });
...
const div2 = div({ class: 'col m6', id: 'div1' });
...
const div3 = div({ class: 'col m6', id: 'div3' });
masterDiv2.appendChild(div3)

const div4 = div({ class: 'col m6', id: 'div3' });
masterDiv2.appendChild(div4);

const div5 = div({ class: 'col m6', id: 'div3' });
masterDiv2.appendChild(div5);

const divButton = div({ class: 'col m6', id: 'div1' });
var divButtonButton = button({ class: 'button', value: 'compare',id: 'divActualButton' }, 'Click Here to Compare');
...
divButtonButton.addEventListener('click', event => {
  ...
  if (input1.value.length > 0&& input2.value.length>0) {
    ...
    Promise.all([obj.scrape(input1.value), obj2.scrape(input2.value)])
      ...
      while(playerArray2.length!==0){
        ...
        console.log(info1)
        console.log(info2);
        var card1 = div({class:'playerCards'},
          div({class:'headImg'},
        ...
        var card2 = div({class:'playerCards'},
          div({class:'headImg'},
        ...
        playerArray1.push(`${info1['name']}`,`${info1['gamesCar']}`,`${info1['pointsCar']}`,`${info1['reboundsCar']}`,`${info1['assistsCar']}`)
        playerArray2.push(`${info2['name']}`,`${info2['gamesCar']}`,`${info2['pointsCar']}`,`${info2['reboundsCar']}`,`${info2['assistsCar']}`)
...
if(typeof playerArray1[0]!=='string'&&typeof playerArray2[0]!=='string'){
  alert("Search for Two Players")
}
  if(typeof playerArray1[0]=='string' && typeof playerArray2[0]=='string'){
    if(Math.max.apply(null,[playerArray1[1],playerArray2[1]])==playerArray1[1]){
      gameString=`${playerArray1[0]} has more Career Games Played`
  }
  if (Math.max.apply(null,[playerArray1[1],playerArray2[1]])==playerArray2[1]){
      gameString=`${playerArray2[0]} has more Career Games Played`
  }

  if(Math.max.apply(null,[playerArray1[2],playerArray2[2]])==playerArray1[2]){
      pointString=`${playerArray1[0]} has more Career PPG`
  }
  if(Math.max.apply(null,[playerArray1[2],playerArray2[2]])==playerArray2[2]){
      pointString=`${playerArray2[0]} has more Career PPG`
  }
   if(Math.max.apply(null,[playerArray1[3],playerArray2[3]])==playerArray1[3]){
      reboundString=`${playerArray1[0]} has more Career RPG`
  }
  if(Math.max.apply(null,[playerArray1[3],playerArray2[3]])==playerArray2[3]){
      reboundString=`${playerArray2[0]} has more Career RPG`
  }
  if(Math.max.apply(null,[playerArray1[4],playerArray2[4]])==playerArray1[4]){
      assistsString=`${playerArray1[0]} has more Career APG`
  }
  if(Math.max.apply(null,[playerArray1[4],playerArray2[4]])==playerArray2[4]){
      assistsString=`${playerArray2[0]} has more Career APG`
  }
    

const compareDiv = div({class:'playerCards'},
